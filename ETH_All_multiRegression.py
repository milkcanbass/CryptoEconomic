...
data_directory = 'Data'
eth_data = load_and_prepare_data('ETH-USD.csv', 'Close', data_directory)
...
for file_name, column_name in datasets.items():
    ...
    if not df.empty:
        eth_data = eth_data.join(df, how='outer', rsuffix='_other')  # Add a suffix to avoid column name clashes
...
eth_data = eth_data.apply(pd.to_numeric, errors='coerce').dropna()

if not eth_data.empty:
    X = eth_data.drop('ETH-USD_Close', axis=1, errors='ignore')
    y = eth_data['ETH-USD_Close']

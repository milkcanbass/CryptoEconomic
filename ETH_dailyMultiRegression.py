...
# Load and prepare data for ETH instead of BTC
eth_data = load_data('ETH-USD.csv', 'Close', 'ETH_Close')
...
eth_data = eth_data.join(data_frames, how='outer')
...
eth_data = eth_data.apply(pd.to_numeric, errors='coerce')
eth_data.dropna(inplace=True)  # Drop any rows that still contain NaN

if not eth_data.empty:
    X = eth_data.drop('ETH_Close', axis=1)
    y = eth_data['ETH_Close']
